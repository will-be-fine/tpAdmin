...
class Menu extends CommonController
{
    ...
    {
        $this->model = new \app\model\AdminMenu();
    }
    ...
    public function getMenuTree(){
        ...
        if($user->is_super_manager === 1){
            ...
            $roleIds = \app\model\role\User::where('admin_user_id','=', $user->id)
                ->column('admin_role_id');
            $menuIds = \app\model\menu\Role::where('admin_role_id','in',$roleIds)
                ->column('admin_menu_id');
            $where[] = ['id','in',$menuIds];
            $sourceData  = $this->model->order($this->orderRule)->where($where)->select()->toArray();
        }
    ...
    public function copyBatch()
    {
        ...
        $data = \app\model\admin\AdminMenu::where('id','in', $ids)
    ...
    public function move()
    {
        ...
        $save = \app\model\admin\AdminMenu::where('id','in', $ids)
            ->save(['pid'=>$pid]);
