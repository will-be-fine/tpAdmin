...
layui.define(["jquery", "element"], function (exports) {
    ...
    var laytpUpload = {
        ...
        uploadHtml: function (options) {
            //防止重复渲染
            if(document.getElementById('id_'+options.name)){
                return false;
            }
            var html =
                ...
                "             上传{{laytpUpload.uploadType[d.accept]}}" +
                "       </a>" +
                "   </div>" +
                "{{# } }}" +
                "{{# if(d.showChoiceBtn){ }}" +
                ...
                if(options.multi){
                    ...
                    "            <div class=\"laytp-btn laytp-btn-danger laytp-btn-md layui-input-block more-upload-clear-btn\" style=\"margin-left: 0px;min-height: auto;\" >\n" +
                    "                            一键清除" +
                    "                        </div>" +
                    "       </div>" ;
                }
        ...
        imagePreviewHtml: function (options) {
            var html =
                ...
                "       <li class=\"item_img layer-photos-demo\">" +
                "           <div class=\"operate\">" +
                "           <i class=\"toleft layui-icon\">《</i><i class=\"toright layui-icon\">》</i><i  class=\"close layui-icon\"></i>"+
                ...
                "           </div>" +
                "           <img layer-src=\"{{uploadedArr[key]}}\" src=\"{{uploadedArr[key]}}\" class=\"img\">" +
                "       </li>" +
                "   {{# } }}" +
                "   {{# } }}" +
                "   </ul>" +
            ...
            layui.upload.render({
                ...
                choose: function (obj) {
                    ...
                    obj.preview(function (index, file, result) {
                        ...
                        //     $.ajax({
                        //         url: "https://unpkg.com/img-compressor/dist/img-compressor.js",
                        //         dataType: "script",
                        //         cache: true
                        //     }).done(function () {
                        //         var optionWidth = 1080;
                        //         if(!options.width){
                        //             optionWidth = options.width;
                        //         }
                        //         var ImageCompressor = new ImgCompressor.Create({
                        //             width: optionWidth,
                        //         });
                        ...
                        //             maxHeight: 1080,
                        //             success(result) {
                        //                 var newfile = new File([result], file.name, {type: file.type})
                        //                 // console.log(result)
                        //                 // console.log(file)
                        //                 // console.log(newfile)
                        //                 options.index = index;
                    ...
                    if(options.uploadedCallback != ''){
                        eval(options.uploadedCallback + "(options)")
                    }
...
//多图上传点击<>左右移动图片
    $(document).on("click",".item_img .toleft",function(){
        $('.laytp-more-upload-list .laytp-more').remove()

        var li_index=$(this).closest("#preview_imgs_fid .item_img").index();
        // var length = $(this).closest("#preview_imgs_fid").find(".item_img").length;

        // console.log(li_index-length)
        if(li_index>=1){
            var input_imgs_fid = $('#input_imgs_fid').val();
            var input_imgs_fid_arr = input_imgs_fid.split(',')
            input_imgs_fid_arr.splice(li_index-1, 0, input_imgs_fid_arr[li_index]);
            input_imgs_fid_arr.splice(li_index+1,1);

            console.log(li_index)
            console.log(input_imgs_fid_arr)
            console.log(input_imgs_fid_arr[li_index])

            $('#input_imgs_fid').val(input_imgs_fid_arr.join(','));




            $(this).closest(".item_img").insertBefore($(this).closest("#preview_imgs_fid").find(".item_img").eq(Number(li_index-length)-1));
        }

    });

    $(document).on("click",".item_img .toright",function(){
        $('.laytp-more-upload-list .laytp-more').remove()
        ...
        input_imgs_fid_arr.splice(li_index+2, 0, input_imgs_fid_arr[li_index]);
        input_imgs_fid_arr.splice(li_index,1);
        ...
        // console.log(input_imgs_fid_arr.join(','))
        ...
        $(this).closest(".item_img").insertAfter($(this).closest("#preview_imgs_fid").find(".item_img").eq(Number(li_index-length)+1));

    });

    $(document).on("click",".more-upload-clear-btn",function(){
        console.log(123456)
        $("#preview_imgs_fid .item_img").remove()
        $('#input_imgs_fid').val('');
    })
